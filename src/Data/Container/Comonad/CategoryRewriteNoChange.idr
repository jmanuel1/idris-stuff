module Data.Container.Comonad.CategoryRewriteNoChange
...
cofreeIsFunctor = MkFunctor {
  ...
  identity = \a => cong Mor (funExt $ \(MkCofree (MkM c)) =>
    cong (MkCofree . MkM) $
    -- c : Extension (MkContainer (a, container .Shape) (\lamc => let (p, q) = lamc in Either (Position (MkContainer a (\value => Void)) p) (Position container q))) (Inf (M (MkContainer (a, container .Shape) (\lamc => let (p, q) = lamc in Either (Position (MkContainer a (\value => Void)) p) (Position container q)))))
    let ffc := (fst (fromPair c)) in
    rewrite constRoundTrip' ?gffda (Inf (M (MkContainer (a, container .Shape) (\lamc => let (p, q) = lamc in Either (Position (Const a) p) (Position container q))))) (fst (fromPair c)) in
